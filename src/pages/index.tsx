...
interface Msg { id: string; role: 'user'|'assistant'; content: string; tool?: ToolType; files?: FileAttachment[]; provider?: string; ts: number; }
...
export default function Home() {
  ...
  const send = useCallback(async () => {
    ...
    if(!text||busy) return;
    const uMsg: Msg = { id:crypto.randomUUID(), role:'user', content:text, ts:Date.now() };
    const aMsg: Msg = { id:crypto.randomUUID(), role:'assistant', content:'', ts:Date.now() };
    setMsgs(p=>[...p,uMsg,aMsg]); setInput(''); setBusy(true);
    ...
    try {
      ...
      const res = await fetch(`${api}/api/chat`, {
        ...
        body: JSON.stringify({ message:text, model:model||undefined, history:msgs.slice(-10).map(m=>({role:m.role,content:m.content})) }),
  ...
  }, [input,busy,model,msgs,activeConv,api]);
  ...
  return (
    <div style={{display:'flex',height:'100dvh',background:v.bg,color:v.text,fontFamily:"'SF Pro Display',-apple-system,sans-serif",overflow:'hidden'}}>
      ...
      <aside style={{width:side?(isMob?'80vw':260):0,maxWidth:side?300:0,background:v.bg2,borderRight:`1px solid ${v.border}`,display:'flex',flexDirection:'column',overflow:'hidden',transition:'all .2s',...(isMob&&side?{position:'fixed',left:0,top:0,bottom:0,zIndex:50}:{})}}>
        ...
        <div style={{padding:'8px 14px',borderTop:`1px solid ${v.border}`,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
      ...
      <main style={{flex:1,display:'flex',flexDirection:'column',minWidth:0}}>
        ...
        <div style={{padding:isMob?'8px 10px 12px':'10px 16px 16px',borderTop:`1px solid ${v.border}`,flexShrink:0,background:v.bg}}>
          <div style={{maxWidth:720,margin:'0 auto',display:'flex',gap:7,alignItems:'flex-end'}}>
            ...
            {busy?<button onClick={()=>{abortRef.current?.abort();setBusy(false)}} style={{padding:'11px 14px',borderRadius:12,background:v.red,border:'none',color:'#fff',cursor:'pointer',fontWeight:600,fontSize:13,flexShrink:0}}>Stop</button>
            :<button onClick={send} disabled={!input.trim()} style={{padding:'11px 14px',borderRadius:12,border:'none',fontWeight:600,fontSize:13,flexShrink:0,background:input.trim()?v.accent:v.bg3,color:input.trim()?'#fff':v.text3,cursor:input.trim()?'pointer':'default'}}>Send</button>}
          </div>
